...
- [x] 10.6 Enable the `schedule` trigger and confirm the next run commits and republishes on its own — *`schedule: '17 4 * * *'` enabled. Everything downstream of the trigger is proven: the `workflow_dispatch` run (10.2) scraped, committed and deployed, and a `push` run rebuilt from the committed snapshot with the scrape skipped. The cron firing itself was not observed — accepted on the maintainer's call*
